...
class FeatureEngineeringPipeline:
    ...
    ### DIKEMBALIKAN SESUAI ASLINYA: FUNGSI SELEKSI FITUR SEBAGAI METODE TERPISAH ###
    def _remove_low_variance_features(self, df: pd.DataFrame) -> List[str]:
        feature_columns = [
            c
            for c in df.select_dtypes(include=np.number).columns
            if c not in ["PERIODE", "AGE", "FCR_ACT"]
        ]
        ...
        selector.fit(df[feature_columns].fillna(0))
        removed_cols = [
            feature_columns[i] for i, v in enumerate(selector.get_support()) if not v
        ]
        logger.info(
            f"Seleksi Varians Rendah mengidentifikasi {len(removed_cols)} kolom untuk dihapus."
        )
        return removed_cols

    ### DIKEMBALIKAN SESUAI ASLINYA: FUNGSI SELEKSI FITUR SEBAGAI METODE TERPISAH ###
    def _remove_highly_correlated_features(self, df: pd.DataFrame) -> List[str]:
        feature_columns = [
            c
            for c in df.select_dtypes(include=np.number).columns
            if c not in ["PERIODE", "AGE", "FCR_ACT"]
        ...
        to_drop = [
            ...
            if any(upper[column] > self.config.get("correlation_threshold", 0.95))
        ]
        logger.info(
            f"Seleksi Korelasi Tinggi mengidentifikasi {len(to_drop)} kolom untuk dihapus."
        )
        return to_drop
    ...
    def fit(self, X: pd.DataFrame, y=None, metadata_output_dir: str = None):
        ...
        # 2. Identifikasi kolom yang akan dihapus menggunakan metode yang sesuai
        removed_low_var = self._remove_low_variance_features(X_with_features)

        # Hapus kolom varians rendah sebelum cek korelasi
        X_temp = X_with_features.drop(columns=removed_low_var, errors="ignore")
        removed_high_corr = self._remove_highly_correlated_features(X_temp)

        # Gabungkan semua kolom yang akan dihapus
        self.columns_to_drop_ = sorted(list(set(removed_low_var + removed_high_corr)))

        # Simpan daftar kolom final
        self.final_columns_ = [
            col for col in X_with_features.columns if col not in self.columns_to_drop_
        ]

        # 3. Simpan metadata jika path disediakan
        if metadata_output_dir:
            removed_features_dict = {
                "low_variance": removed_low_var,
                "high_correlation": removed_high_corr,
            }
            self._save_feature_metadata(metadata_output_dir, removed_features_dict)

        self.is_fitted = True
        logger.info(
            f"Proses 'fit' selesai. Total {len(self.columns_to_drop_)} kolom unik diidentifikasi untuk dihapus."
        )
        return self
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        df_transformed = df_with_features.drop(
            columns=self.columns_to_drop_, errors="ignore"
        )
        ...
        df_final = df_transformed[self.final_columns_]
...
# =============================================================================
# BLOK UNTUK MEMBUAT, MELATIH, DAN MENYIMPAN MODEL PIPELINE
# =============================================================================
if __name__ == "__main__":
    print("Membuat, melatih (fit), dan menyimpan model FeatureEngineeringPipeline...")
    input_file_path = os.path.join("Normalisasi", "Data", "File_Gabungan.csv")
    output_model_path = os.path.join("models", "feature_engineering_pipeline.pkl")
    output_data_path = os.path.join(
        "Normalisasi", "Data", "File_Gabungan_Enhanced_Features.csv"
    )
    metadata_folder = os.path.join("Hasil Normalisasi")

    os.makedirs("models", exist_ok=True)
    os.makedirs(os.path.join("Normalisasi", "Data"), exist_ok=True)
    ...
    if not os.path.exists(input_file_path):
        ...
        fe_pipeline = FeatureEngineeringPipeline(config=FEATURE_CONFIG)
        df_enhanced = fe_pipeline.fit_transform(
            df_raw, metadata_output_dir=metadata_folder
        )
        df_enhanced.to_csv(output_data_path, sep=";", index=False)
        print(f"Data dengan fitur enhanced berhasil disimpan di: {output_data_path}")
        with open(output_model_path, "wb") as f:
            pickle.dump(fe_pipeline, f)
        print(
            f"Model FeatureEngineeringPipeline yang sudah di-'fit' berhasil disimpan di: {output_model_path}"
        )
